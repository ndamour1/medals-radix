...
function App() {
  ...
  const apiCountryEndpoint = "https://medalsapi.azurewebsites.net/api/country";
  const medals = useRef([
  ...
  const handleAdd = async (name, gold, silver, bronze) => {
    const { data: post } = await axios.post(apiCountryEndpoint, {
      name: name,
      gold: gold,
      silver: silver,
      bronze: bronze,
  ...
  const handleDelete = async (countryId) => {
    ...
    try {
      await axios.delete(`${apiCountryEndpoint}/${countryId}`);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const { data: fetchedCountries } = await axios.get(apiCountryEndpoint);
